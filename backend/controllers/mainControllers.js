...
const { sendOrderConfirmedEmail, sendOrderShippedEmail, sendOrderDeliveredEmail } = require('../utils/emailService');
const { sendOrderConfirmedSMS, sendOrderShippedSMS, sendOrderDeliveredSMS } = require('../utils/smsService');
...
exports.createOrder = async (req, res) => {
  try {
    ...
    for (const item of orderItems) {
      ...
      if (product.stock < item.quantity) return res.status(400).json({ success: false, message: `Only ${product.stock} units of "${product.name}" available` });
...
exports.getOrder = async (req, res) => {
  try {
    const order = await Order.findById(req.params.id).populate('user', 'name email').populate('orderItems.product', 'name images price');
...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    if (orderStatus === 'Delivered') { order.deliveredAt = Date.now(); order.isPaid = true; }
...
// RESET revenue — zeroes out price fields on all orders (admin only)
...
// ── NEW: Seller deletes only THEIR OWN orders permanently ─────────
// DELETE /api/orders/seller/my-orders
exports.deleteMyOrders = async (req, res) => {
  try {
    ...
    // Delete orders where at least one item belongs to this seller's products
    const result = await Order.deleteMany({
...
exports.getCart = async (req, res) => {
  try {
    let cart = await Cart.findOne({ user: req.user._id }).populate('items.product', 'name images price stock sizes colors');
...
exports.getWishlist = async (req, res) => {
  try {
    let wishlist = await Wishlist.findOne({ user: req.user._id }).populate('products', 'name images price discountPrice ratings category');
