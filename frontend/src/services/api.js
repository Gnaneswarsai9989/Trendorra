...
export const orderAPI = {
  ...
  deleteMyOrders: ()       => API.delete('/orders/seller/my-orders'), // ← ADD THIS
};

// ── Delivery (Prototype → Real Delhivery) ─────────────────────────
export const deliveryAPI = {
  // Seller: mark order packed & ready, triggers Delhivery pickup
  markReady:   (orderId) => API.post(`/delivery/ready/${orderId}`),

  // PROTOTYPE ONLY: simulate next delivery step (Processing→Confirmed→Shipped→…→Delivered)
  // Remove this when going live with real Delhivery
  simulate:    (orderId) => API.post(`/delivery/simulate/${orderId}`),

  // Track shipment by Delhivery waybill number
